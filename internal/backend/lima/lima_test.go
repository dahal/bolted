...
// interface even while it's still a stub. Spec 05 must not regress this.
...
// nasty foot-gun for the spec-05 implementer.
func TestBackend_AllMethodsReturnNotImplemented(t *testing.T) {
	...
	for _, c := range checks {
		...
		if !strings.Contains(err.Error(), "spec 05") {
			t.Errorf("%s: expected error to reference spec 05, got: %v", c.name, err)
		}
		if !strings.Contains(err.Error(), "lima") {
			t.Errorf("%s: expected error to mention lima, got: %v", c.name, err)
		}
	}
}
